...
class Serial:
    ...
    def setRTS(self, value):
        controlByte = cpu.readModemControlRegister(self.port)
        if value:
            controlByte = controlByte | 0x02
        else:
            controlByte = controlByte & 0xFD
        cpu.writeModemControlRegister(self.port, controlByte)

    def setDTR(self, value):
        controlByte = cpu.readModemControlRegister(self.port)
        if value:
            controlByte = controlByte | 0x01
        else:
