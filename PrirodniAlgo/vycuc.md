...
**Evoluce vah** – pevná architektura, hledají se jen váhy; jedinec = vektor vah → spojitá optimalizace; ev. strategie (+N(0,σ) ke vahám)
- proč evolucí: funguje bez gradientu / u RL (jen fitness, ne žádané výstupy)

**NEAT** – vyvíjí váhy I topologii, síť roste z minimální
- jedinec = seznam genů spojů: odkud, kam, váha, aktivní/neaktivní, inovační číslo
- mutace: přidání spoje, přidání neuronu (rozdělí spoj) + mutace vah, křížení

**Inovační čísla** – globálně unikátní ID přidělené při vzniku nového spoje = "kdy vznikl"
- stejné č. = stejný historický původ = spárovat při křížení (řeší competing conventions)
- matching geny → náhodně od rodiče; disjoint/excess → od zdatnějšího
- slouží i k měření podobnosti → speciation (ochrana nových topologií)

**HyperNEAT** – nepřímé kódování: NEAT vyvíjí CPPN, ta z dvojice souřadnic neuronů vrací váhu
- využívá geometrii, generuje obří sítě z malého genomu

**coDeepNEAT** – NAS přes koevoluci: populace modulů + populace blueprintů; složená síť se natrénuje backpropem, výkon = fitness

**Novelty search** – odměňuje novost chování (vzdálenost od archivu viděných chování), ne cíl
- řeší klamavé (deceptive) problémy, kde cílená fitness uvázne v lok. optimu
- vs. fitness: + uniká klamům, diverzita; − na snadných problémech bloudí, plýtvá; často kombinace obou

## 14. Zpětnovazební učení

**Intuice** – agent koná akce v prostředí, dostává odměny, hledá strategii π(s)→a max. celkovou odměnu; správná akce není dána, objevuje se zkoušením

**MDP** – $(S, A, P, R)$: stavy, akce, přechody $P(s'|s,a)$, odměny $R(s,a,s')$
- Markovská vlastnost: budoucnost závisí jen na aktuálním stavu, ne na historii

**Cíl** – max. očekávaný diskontovaný součet:
$$G_t = \sum_{k=0}^{\infty} \gamma^k R_{t+k+1}, \quad \gamma\in[0,1)$$

**Hodnoty**
- $V^\pi(s)$ – očekávaná odměna od stavu $s$ při strategii π
- $Q^\pi(s,a)$ – očekávaná odměna za akci $a$ v $s$, pak π; nejlepší akce = $\arg\max_a Q(s,a)$

**Monte Carlo** – odehraj celé epizody, hodnota = průměr skutečných $G_t$; musí čekat na konec epizody

**Q-učení (POVINNÝ VZOREC)** – off-policy, TD, bez modelu:
$$Q(s,a) \leftarrow Q(s,a) + \alpha\big[r + \gamma \max_{a'} Q(s',a') - Q(s,a)\big]$$
- cíl = $r + \gamma\max_{a'}Q(s',a')$, TD chyba = cíl − Q(s,a)
- $\max$ → učí optimální strategii nezávisle na chování

**SARSA** – on-policy, místo max skutečně zvolená $a'$:
$$Q(s,a) \leftarrow Q(s,a) + \alpha\big[r + \gamma Q(s',a') - Q(s,a)\big]$$
- z $(s,a,r,s',a')$; učí hodnotu reálně používané strategie; opatrnější

**Spojité stavy/akce**
- spojité stavy → aproximátor funkce (lin. / neuronka = hluboké Q-učení)
- spojité akce → diskretizace, nebo actor-critic/DDPG (akci produkuje síť, ne max)

## 15. Hluboké zpětnovazební učení

**Hluboké Q-učení (DQN)** – tabulku $Q$ nahradí síť $Q_\theta(s,a)$; trénink na Bellmanův cíl:
$$L(\theta) = \big(r + \gamma\max_{a'}Q_\theta(s',a') - Q_\theta(s,a)\big)^2$$
- update = gradientní krok na vahách (místo zápisu do tabulky)

**Tabulkové vs. hluboké** – tabulka: málo diskrétních stavů, spolehlivé; síť: generalizuje na velké/spojité stavy, ale nestabilní

**Experience replay** – přechody $(s,a,r,s')$ do bufferu, trénink na náhodných dávkách
- rozbije korelaci po sobě jdoucích přechodů + opakované využití dat

**Target network** – zamražená kopie $Q_{\theta^-}$ pro výpočet cíle:
$$L(\theta)=\big(r+\gamma\max_{a'}Q_{\theta^-}(s',a') - Q_\theta(s,a)\big)^2$$
- $\theta^-$ se kopíruje z $\theta$ jen občas → stabilní cíl (nehoní se za vlastním ocasem)

**Vliv triků** – bez nich DQN nestabilní; replay řeší korelaci dat, target net řeší pohyblivý cíl

**DDPG** – spojité akce; actor $\mu(s)$ produkuje akci (místo max) + critic $Q(s,a)$; replay + target sítě; robotika

**Actor-critic** – actor učí strategii $\pi(a|s)$, critic učí hodnotu ($V$/$Q$) a hodnotí actora; spojité akce + nižší rozptyl

**Advantage** – $A(s,a) = Q(s,a) - V(s)$
- relativní kvalita akce vůči průměru stavu; odečtení $V(s)$ sníží rozptyl gradientu → stabilnější učení

**A3C** – asynchronní advantage actor-critic; víc agentů paralelně do sdílených parametrů
- rozmanité nekorelované zkušenosti → dekorelace bez replay bufferu, rychlé
...
**Typy (médium)**
- Soft – softwarový (CA, Tierra)
- Hard – hardwarový (roboti)
- Wet – biochemický (umělé buňky, syntetická biologie)

**Silný vs. slabý**
- Slabý – jen model/simulace života, nástroj ke studiu
- Silný – umělý život je skutečně živý (život = organizace, ne hmota)

**Celulární automaty** – mřížka buněk se stavy, diskrétní čas, nový stav z pravidla podle sousedů, aktualizace současně
- 1D – řada, sousedi vlevo/vpravo (Wolframova pravidla, Rule 110)
- 2D – mřížka, 8 sousedů (Mooreovo okolí) → Game of Life

**Game of Life** (8 sousedů)
- živá s 2–3 sousedy přežívá
- živá s <2 (osamělost) nebo >3 (přelidnění) umírá
- mrtvá s přesně 3 ožívá
- útvary: stabilní, oscilátory, gliders; Turingovsky úplný

**Langtonův mravenec**
- bílé pole: doprava 90°, přebarvi na černo, krok
- černé pole: doleva 90°, přebarvi na bílo, krok
- dálnice (highway) – po ~10k krocích chaosu začne stavět cyklický vzor (104 kroků) táhnoucí ho do nekonečna; emergentní řád

**Tierra** – evoluce assemblerových programů v paměti virt. počítače, soutěž o CPU+paměť; mutace při kopírování; otevřená evoluce bez fitness funkce

**Jedinec v Tierra** – program v assembleru se sebekopírovací rutinou; život = CPU čas, smrt = smazání správcem paměti

**Adresování (skoky)** – podle vzoru (template matching, ne absolutní adresa): skok udá vzor no-opů, procesor najde komplementární vzor a skočí; robustní vůči mutacím/posunům kódu

**Paraziti** – programy bez vlastní kopírovací rutiny, přes vzorové adresování využijí cizí hostitelskou rutinu; kratší → rychlejší množení; → hyperparaziti, závody ve zbrojení
